...
func main() {
	...
	logx.Info("Starting neurons API Server...")
...
func registerRoutes(app *fiber.App, container *Container) {
	...
	// Gamification (Neurons) routes. Order matters: modules that own
	// /me/... routes are registered after the classroom-scoped ones.
	mw := container.IAM.UnifiedAuthMiddleware
	container.Classroom.RegisterRoutes(protected, mw)
	container.Enrollment.RegisterRoutes(protected, mw)
	container.Team.RegisterRoutes(protected, mw)
	container.Reason.RegisterRoutes(protected, mw)
	container.Benefit.RegisterRoutes(protected, mw)
	container.Medal.RegisterRoutes(protected, mw)
	container.Ledger.RegisterRoutes(protected, mw)
	container.QR.RegisterRoutes(protected, mw)
	logx.Info("  > Gamification routes registered")
