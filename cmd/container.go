...
import (
	...
	"github.com/Abraxas-356/neurons/internal/fsx"
	"github.com/Abraxas-356/neurons/internal/fsx/fsxlocal"
	"github.com/Abraxas-356/neurons/internal/fsx/fsxs3"
	"github.com/Abraxas-356/neurons/internal/gamification/benefit/benefitcontainer"
	"github.com/Abraxas-356/neurons/internal/gamification/classroom/classroomcontainer"
	"github.com/Abraxas-356/neurons/internal/gamification/enrollment/enrollmentcontainer"
	"github.com/Abraxas-356/neurons/internal/gamification/ledger/ledgercontainer"
	"github.com/Abraxas-356/neurons/internal/gamification/medal/medalcontainer"
	"github.com/Abraxas-356/neurons/internal/gamification/qr/qrcontainer"
	"github.com/Abraxas-356/neurons/internal/gamification/reason/reasoncontainer"
	"github.com/Abraxas-356/neurons/internal/gamification/team/teamcontainer"
	"github.com/Abraxas-356/neurons/internal/gamification/userlookup"
	"github.com/Abraxas-356/neurons/internal/iam/iamcontainer"
	"github.com/Abraxas-356/neurons/internal/jobx"
	"github.com/Abraxas-356/neurons/internal/jobx/jobxredis"
	"github.com/Abraxas-356/neurons/internal/kernel"
	"github.com/Abraxas-356/neurons/internal/logx"
	"github.com/Abraxas-356/neurons/internal/notifx"
	"github.com/Abraxas-356/neurons/internal/notifx/notifxconsole"
	"github.com/Abraxas-356/neurons/internal/notifx/notifxses"
...
type Container struct {
	...
	Classroom    *classroomcontainer.Container
	Enrollment   *enrollmentcontainer.Container
	Team         *teamcontainer.Container
	Reason       *reasoncontainer.Container
	Benefit      *benefitcontainer.Container
	Medal        *medalcontainer.Container
	Ledger       *ledgercontainer.Container
	QR           *qrcontainer.Container
...
func (c *Container) initModules() {
	...
	c.Classroom = classroomcontainer.New(classroomcontainer.Deps{
		DB:    c.DB,
		Users: userlookup.New(c.IAM.UserRepository),
	})

	c.Enrollment = enrollmentcontainer.New(enrollmentcontainer.Deps{
		DB:         c.DB,
		Classrooms: c.Classroom.ClassroomService,
		Users:      userlookup.New(c.IAM.UserRepository),
	})

	c.Team = teamcontainer.New(teamcontainer.Deps{
		DB:          c.DB,
		Classrooms:  c.Classroom.ClassroomService,
		Enrollments: c.Enrollment.EnrollmentService,
	})

	c.Reason = reasoncontainer.New(reasoncontainer.Deps{
		DB:         c.DB,
		Classrooms: c.Classroom.ClassroomService,
	})

	c.Benefit = benefitcontainer.New(benefitcontainer.Deps{
		DB:          c.DB,
		Classrooms:  c.Classroom.ClassroomService,
		Enrollments: c.Enrollment.EnrollmentService,
	})

	c.Medal = medalcontainer.New(medalcontainer.Deps{
		DB:          c.DB,
		Classrooms:  c.Classroom.ClassroomService,
		Enrollments: c.Enrollment.EnrollmentService,
	})

	c.Ledger = ledgercontainer.New(ledgercontainer.Deps{
		DB:          c.DB,
		Classrooms:  c.Classroom.ClassroomService,
		Enrollments: c.Enrollment.EnrollmentService,
		Reasons:     c.Reason.ReasonService,
		Benefits:    c.Benefit.BenefitService,
	})

	c.QR = qrcontainer.New(qrcontainer.Deps{
		Redis:       c.Redis,
		Classrooms:  c.Classroom.ClassroomService,
		Enrollments: c.Enrollment.EnrollmentService,
		Ledger:      c.Ledger.LedgerService,
	})
...
func (c *Container) initFileStorage() {
	...
	case "s3":
		...
		awsBucket := getEnv("AWS_BUCKET", "neurons-uploads")
